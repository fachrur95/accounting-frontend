...
export const defaultUndefinedResult: PaginationResponse<ITerm> = {
  rows: [],
  countRows: 0,
  countAll: 0,
  currentPage: 0,
...
const GLOBAL_URL = `${env.BACKEND_URL}/v1/terms`;
...
export const termRouter = createTRPCRouter({
  findAll: protectedProcedure.input(
    ...
    const result = await axios.get<PaginationResponse<ITerm>>(
      url,
      {
  ...
  findOne: protectedProcedure.input(
    ...
    const result = await axios.get<ITerm>(
      `${GLOBAL_URL}/${input.id}`,
      {
        withCredentials: true,
        headers: { Authorization: `Bearer ${ctx.session.accessToken}` },
  ...
  create: protectedProcedure.input(
    ...
    try {
      const result = await axios.post<ITerm>(
        `${GLOBAL_URL}`,
        input,
        {
          withCredentials: true,
  ...
  update: protectedProcedure.input(
    ...
    const { id, ...data } = input
    try {
      const result = await axios.patch<ITerm>(
        `${GLOBAL_URL}/${id}`,
        data,
        {
          withCredentials: true,
  ...
  destroy: protectedProcedure.input(
    ...
    try {
      const result = await axios.delete<ITerm>(
        `${GLOBAL_URL}/${input.id}`,
        {
          withCredentials: true,
          headers: { Authorization: `Bearer ${ctx.session.accessToken}` },
