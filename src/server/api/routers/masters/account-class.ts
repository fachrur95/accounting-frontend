...
export const defaultUndefinedResult: PaginationResponse<IAccountClass> = {
  rows: [],
  countRows: 0,
  countAll: 0,
  currentPage: 0,
...
const GLOBAL_URL = `${env.BACKEND_URL}/v1/account-classes`;

export const accountClassRouter = createTRPCRouter({
  findAll: protectedProcedure.input(
    ...
    const result = await axios.get<PaginationResponse<IAccountClass>>(
      url,
      {
  ...
  findOne: protectedProcedure.input(
    ...
    const result = await axios.get<IAccountClass>(
      `${GLOBAL_URL}/${input.id}`,
      {
        withCredentials: true,
        headers: { Authorization: `Bearer ${ctx.session.accessToken}` },
  ...
  create: protectedProcedure.input(
    ...
    try {
      const result = await axios.post<IAccountClass>(
        `${GLOBAL_URL}`,
        input,
        {
          withCredentials: true,
  ...
  update: protectedProcedure.input(
    ...
    const { id, ...data } = input
    try {
      const result = await axios.patch<IAccountClass>(
        `${GLOBAL_URL}/${id}`,
        data,
        {
          withCredentials: true,
  ...
  destroy: protectedProcedure.input(
    ...
    try {
      const result = await axios.delete<IAccountClass>(
        `${GLOBAL_URL}/${input.id}`,
        {
          withCredentials: true,
          headers: { Authorization: `Bearer ${ctx.session.accessToken}` },
