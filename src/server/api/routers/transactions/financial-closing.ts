...
export const defaultUndefinedResult: PaginationResponse<IFinancialClosing> = {
  rows: [],
  countRows: 0,
  countAll: 0,
  currentPage: 0,
...
const GLOBAL_URL = `${env.BACKEND_URL}/v1/financial-closings`;

export const financialClosingRouter = createTRPCRouter({
  findAll: protectedProcedure.input(
    ...
    const result = await axios.get<PaginationResponse<IFinancialClosing>>(
      url,
      {
  ...
  findOne: protectedProcedure.input(
    ...
    const result = await axios.get<IFinancialClosing>(
      `${GLOBAL_URL}/${input.id}`,
      {
        withCredentials: true,
        headers: { Authorization: `Bearer ${ctx.session.accessToken}` },
  ...
  create: protectedProcedure.input(
    ...
    try {
      const result = await axios.post<IFinancialClosing>(
        `${GLOBAL_URL}`,
        input,
        {
          withCredentials: true,
  ...
  update: protectedProcedure.input(
    ...
    const { id, ...data } = input;
    try {
      const result = await axios.patch<IFinancialClosing>(
        `${GLOBAL_URL}/${id}`,
        data,
        {
          withCredentials: true,
  ...
  destroy: protectedProcedure.input(
    ...
    try {
      const result = await axios.delete<IFinancialClosing>(
        `${GLOBAL_URL}/${input.id}`,
        {
          withCredentials: true,
          headers: { Authorization: `Bearer ${ctx.session.accessToken}` },
