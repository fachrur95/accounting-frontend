...
const title = "Bagan Akun";
const path: WorkerPathType = "chart-of-account";

const pathname = "/masters/chart-of-accounts";

const ChartOfAccountsPage: MyPage = () => {
  const router = useRouter();

  const [rows, setRows] = useState<IChartOfAccount[]>([]);
  ...
  const mutationDelete = api.chartOfAccount.destroy.useMutation({
    onSuccess: () => {
      void refetch();
    },
  });
  ...
  useEffect(() => {
    if (data) {
      const dataRows: IChartOfAccount[] = data?.pages
        .map((page) => page.rows.map((row: IChartOfAccount) => row))
        .flat();
...
ChartOfAccountsPage.Layout = "Dashboard";
