...
const MasterAccountSubClassForm = dynamic(
  () => import("@/components/forms/MasterAccountSubClassForm"),
);
...
const AccountSubClassesPage: MyPage = () => {
  const router = useRouter();

  const [rows, setRows] = useState<IAccountSubClass[]>([]);
  ...
  const mutationDelete = api.accountSubClass.destroy.useMutation({
    onSuccess: () => {
      void refetch();
    },
  });
  ...
  const columns: GridColDef[] = [
    {
      ...
      valueGetter: (
        ...
        return params.row.accountClass?.name ?? "-";
      },
    },
    {
      field: "code",
    ...
    },
    {
      ...
      width: 80,
      renderCell: (
        params: GridRenderCellParams<unknown, IAccountSubClass, unknown>,
      ) => {
  ...
  useEffect(() => {
    if (data) {
      const dataRows: IAccountSubClass[] = data?.pages
        .map((page) => page.rows.map((row: IAccountSubClass) => row))
        .flat();
...
export default AccountSubClassesPage;
AccountSubClassesPage.Layout = "Dashboard";
