...
const path: WorkerPathType = "cash-register";

const pathname = "/masters/other/cash-registers";

const CashRegistersPage: MyPage = () => {
  const router = useRouter();

  const [rows, setRows] = useState<ICashRegister[]>([]);
  ...
  const {
    ...
    refetch,
    isFetching,
  } = api.cashRegister.findAll.useInfiniteQuery(
    {
      limit: 50,
      search,
      filter: filterModel,
    ...
    {
      getNextPageParam: (lastPage: PaginationResponse<ICashRegister>) =>
  ...
  const mutationDelete = api.cashRegister.destroy.useMutation({
    onSuccess: () => {
      void refetch();
    },
  });
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: "beginBalanceAccount.name",
  ...
  useEffect(() => {
    if (data) {
      const dataRows: ICashRegister[] = data?.pages
        .map((page) => page.rows.map((row: ICashRegister) => row))
        .flat();
