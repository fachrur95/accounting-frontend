...
const pathname = "/masters/other/taxes";
...
const TaxesPage: MyPage = () => {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = api.tax.findAll.useInfiniteQuery(
    {
      limit: 150,
      search,
      filter: filterModel,
    ...
    {
      getNextPageParam: (lastPage: PaginationResponse<ITax>) =>
  ...
  const mutationDelete = api.tax.destroy.useMutation({
    onSuccess: () => {
      void refetch();
    },
  });
  ...
  useEffect(() => {
    if (data) {
      const dataRows: ITax[] = data?.pages
        .map((page) => page.rows.map((row: ITax) => row))
        .flat();
