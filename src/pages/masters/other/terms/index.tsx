...
const pathname = "/masters/other/terms";

const TermsPage: MyPage = () => {
  const router = useRouter();

  const [rows, setRows] = useState<ITerm[]>([]);
  ...
  const {
    ...
    refetch,
    isFetching,
  } = api.term.findAll.useInfiniteQuery(
    {
      limit: 50,
      search,
      filter: filterModel,
    ...
    {
      getNextPageParam: (lastPage: PaginationResponse<ITerm>) =>
  ...
  const mutationDelete = api.term.destroy.useMutation({
    onSuccess: () => {
      void refetch();
    },
  });
  ...
  useEffect(() => {
    if (data) {
      const dataRows: ITerm[] = data?.pages
        .map((page) => page.rows.map((row: ITerm) => row))
        .flat();
