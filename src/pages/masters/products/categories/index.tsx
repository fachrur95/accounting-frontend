...
// import MasterItemCategoryForm from "@/components/forms/MasterItemCategoryForm";
...
const path: WorkerPathType = "item-category";

const pathname = "/masters/products/categories";

const ItemCategoriesPage: MyPage = () => {
  const router = useRouter();

  const [rows, setRows] = useState<IItemCategory[]>([]);
  ...
  const mutationDelete = api.itemCategory.destroy.useMutation({
    onSuccess: () => {
      void refetch();
    },
  });
  ...
  useEffect(() => {
    if (data) {
      const dataRows: IItemCategory[] = data?.pages
        .map((page) => page.rows.map((row: IItemCategory) => row))
        .flat();
      ...
      <Box className="flex flex-col gap-2">
        ...
        >
          ...
          <DataGridProAdv
            ...
            /* onRowDoubleClick={(params: GridCellParams<unknown, IItemCategory, unknown>) =>
              router.push(
                {
                  pathname,
...
ItemCategoriesPage.Layout = "Dashboard";
