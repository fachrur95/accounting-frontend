...
// import MasterItemTypeForm from "@/components/forms/MasterItemTypeForm";
...
const MasterItemTypeForm = dynamic(
  () => import("@/components/forms/MasterItemTypeForm"),
);

const sortDefault: GridSortModel = [{ field: "name", sort: "asc" }];

const title = "Tipe Produk";
const path: WorkerPathType = "item-type";

const pathname = "/masters/products/item-types";

const ItemTypesPage: MyPage = () => {
  const router = useRouter();

  const [rows, setRows] = useState<IItemType[]>([]);
  ...
  const mutationDelete = api.itemType.destroy.useMutation({
    onSuccess: () => {
      void refetch();
    },
  });
  ...
  useEffect(() => {
    if (data) {
      const dataRows: IItemType[] = data?.pages
        .map((page) => page.rows.map((row: IItemType) => row))
        .flat();
...
ItemTypesPage.Layout = "Dashboard";
