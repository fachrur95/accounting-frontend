...
const pathname = "/masters/products/unit-of-measures";

const UnitOfMeasuresPage: MyPage = () => {
  const router = useRouter();

  const [rows, setRows] = useState<IUnitOfMeasure[]>([]);
  ...
  const mutationDelete = api.unitOfMeasure.destroy.useMutation({
    onSuccess: () => {
      void refetch();
    },
  });
  ...
  useEffect(() => {
    if (data) {
      const dataRows: IUnitOfMeasure[] = data?.pages
        .map((page) => page.rows.map((row: IUnitOfMeasure) => row))
        .flat();
