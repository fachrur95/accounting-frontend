...
const path: WorkerPathType = "user";

const pathname = "/settings/users";

const UsersPage: MyPage = () => {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = api.user.findAll.useInfiniteQuery(
    {
      limit: 50,
      search,
      filter: filterModel,
    ...
    {
      getNextPageParam: (lastPage: PaginationResponse<IUser>) =>
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      headerName: "Email Terverifikasi",
      type: "boolean",
      filterable: false,
      flex: 1,
      minWidth: 200,
      renderCell: (params: GridRenderCellParams<unknown, IUser, unknown>) => (
        <Chip
          ...
          label={params.row.isEmailVerified ? "Ya" : "Tidak"}
          color={params.row.isEmailVerified ? "primary" : "secondary"}
        />
      ),
    },
    {
  ...
  useEffect(() => {
    if (data) {
      const dataRows: IUser[] = data?.pages
        .map((page) => page.rows.map((row: IUser) => row))
        .flat();
