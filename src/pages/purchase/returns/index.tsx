...
const ReturnForm = dynamic(
  () => import("@/components/forms/transactions/ReturnForm"),
);

const sortDefault: GridSortModel = [{ field: "entryDate", sort: "desc" }];

const title = "Return Pembelian";
const path: WorkerPathType = "transaction";

const pathname = "/purchase/returns";

const PurchaseReturnsPage: MyPage<{ userSession: Session["user"] }> = ({
  userSession,
}) => {
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: "transactionParent.transactionNumber",
      headerName: "No. Invoice",
      type: "string",
      flex: 1,
      minWidth: 200,
      valueGetter: (params: GridValueGetterParams<unknown, ITransaction>) => {
        return params.row.transactionParent?.transactionNumber ?? "-";
      },
    },
    {
      field: "people.name",
