...
// import FinancialClosingForm from "@/components/forms/transactions/FinancialClosingForm";
import type { FormSlugType } from "@/types/global";
import type { IJwtDecode } from "@/types/session";
import ConfirmationDialog from "@/components/dialogs/ConfirmationDialog";
import type { WorkerPathType } from "@/types/worker";
import dynamic from "next/dynamic";
import type { IFinancialClosing } from "@/types/prisma-api/financial-closing";
import { Role } from "@/types/prisma-api/role.d";
import type { Session } from "next-auth";

const FinancialClosingForm = dynamic(
  () => import("@/components/forms/transactions/FinancialClosingForm"),
);

const sortDefault: GridSortModel = [{ field: "entryDate", sort: "desc" }];

const title = "Tutup Buku";
const path: WorkerPathType = "financial-closing";

const pathname = "/other-transactions/financial-closings";

const FinancialClosingsPage: MyPage<{ userSession: Session["user"] }> = ({
  userSession,
}) => {
  const router = useRouter();

  const [rows, setRows] = useState<IFinancialClosing[]>([]);
  ...
  const {
    ...
    refetch,
    isFetching,
  } = api.financialClosing.findAll.useInfiniteQuery(
    {
      limit: 50,
      search,
      filter: filterModel,
    ...
    {
      getNextPageParam: (lastPage: PaginationResponse<IFinancialClosing>) =>
  ...
  const mutationDelete = api.financialClosing.destroy.useMutation({
    onSuccess: () => {
      void refetch();
    },
  });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 80,
      renderCell: (
        params: GridRenderCellParams<unknown, IFinancialClosing, unknown>,
      ) => {
  ...
  useEffect(() => {
    if (data) {
      const dataRows: IFinancialClosing[] = data?.pages
        .map((page) => page.rows.map((row: IFinancialClosing) => row))
        .flat();
      ...
      <Box className="flex flex-col gap-2">
        ...
        >
          ...
          <DataGridProAdv
            ...
            /* onRowDoubleClick={(params: GridCellParams<unknown, IFinancialClosing, unknown>) =>
              router.push(
                {
                  pathname,
...
export default FinancialClosingsPage;
FinancialClosingsPage.Layout = "Dashboard";
