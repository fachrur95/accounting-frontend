...
const useInfiniteWarehouse = () => {
  const { ref, inView } = useInView();
  const [search, setSearch] = useState<string>("");
  const [options, setOptions] = useState<IDataOption[]>([]);
  const [countAll, setCountAll] = useState<number>(0);
  const { data, hasNextPage, fetchNextPage, isFetching } =
    api.warehouse.findAll.useInfiniteQuery(
      { limit: 25, search },
      {
        getNextPageParam: (lastPage: PaginationResponse<IWarehouse>) =>
  ...
  const renderOption = (
    ...
    return (
      <li {...props}>
        <div className="warehouses-center flex w-full justify-between">
          {option.label}
          <Box
            component={Done}
...
export default useInfiniteWarehouse;
