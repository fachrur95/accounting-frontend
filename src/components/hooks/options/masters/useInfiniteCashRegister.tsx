...
const useInfiniteCashRegister = () => {
  ...
  const renderOption = (
    ...
    return (
      <li {...props}>
        <div className="cashregisters-center flex w-full justify-between">
          {option.label}
          <Box
            component={Done}
  ...
  useEffect(() => {
    if (data) {
      const dataOptions: IDataOption[] = data?.pages
        .map((page) =>
          page.rows.map((row: ICashRegister) => ({
            id: row.id,
            label: row.name ?? "-",
          })),
        )
...
export default useInfiniteCashRegister;
